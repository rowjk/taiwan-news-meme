...
# 【更新】所有關鍵字已整合成單一行字串
default_stopwords = "的,了,在,是,我,有,和,就,不,人,都,一個,上,也,很,到,說,要,去,會,著,沒有,看,好,自己,這,那,與,及,等,之,所,台灣,報導,新聞,影,曝光,網友,回應,表示,畫面,文章,來源,內容,全文,連結,網址,更多,部分,針對,指出,Yahoo,ETtoday,TVBS,聯合報,中時,自由時報,三立,壹蘋,鏡週刊,NOWnews,公視,華視,東森,民視,台視,中天,中央社,風傳媒,新頭殼,今日新聞,聯合新聞網,中時電子報,UDN,LTN,SETN,CNA,奇摩,新聞網,NEWS,news,GeneOnline,com,ludens,自由,聯合,時報,特報,富房網,娛樂,社會,頭條,Storm,東森新聞,公開,娛樂星聞,mg,tw,體育,縣市,最大,最小,震撼,VR"
...
SENTENCE_TEMPLATES = [
    ...
    # --- 第二批：全新文案 (超狂分類名稱) ---
    # 動漫動漫動動漫
    ("動漫動漫動動漫", "別吵，我正在跟 {{關鍵字}} 進行一場史詩級的 Boss 戰。"),
    ("動漫動漫動動漫", "這款遊戲的 SSR 掉落率太低了，我抽了兩百次只抽到 {{關鍵字}}。"),
    ("動漫動漫動動漫", "隊友掛機就算了，為什麼他的 ID 要叫「狂暴 {{關鍵字}}」？"),
    ("動漫動漫動動漫", "顯卡燒壞了，因為我試圖用 8K 畫質渲染 {{關鍵字}}。"),
    ("動漫動漫動動漫", "教練，我想打籃球，但我手上只有 {{關鍵字}}。"),
    ("動漫動漫動動漫", "直到我的膝蓋中了一箭，我才停止追尋 {{關鍵字}} 的冒險。"),
    ("動漫動漫動動漫", "當我拿出 +9 的 {{關鍵字}}，全伺服器的人都沉默了。"),
    ("動漫動漫動動漫", "現實生活太爛了，我要登出人生去尋找 {{關鍵字}}。"),
    ("動漫動漫動動漫", "這個 NPC 卡住了，一直在重複說著關於 {{關鍵字}} 的台詞。"),
    ("動漫動漫動動漫", "任天堂最新專利：用 {{關鍵字}} 來控制瑪利歐賽車。"),

    # 高流量網紅往紅
    ("高流量網紅往紅", "今天來開箱粉絲送的禮物，天啊！竟然是限量的 {{關鍵字}}！"),
    ("高流量網紅往紅", "按讚訂閱開啟小鈴鐺，不然今晚 {{關鍵字}} 會爬到你床上。"),
    ("高流量網紅往紅", "加上這個濾鏡後，連 {{關鍵字}} 看起來都變得眉清目秀了。"),
    ("高流量網紅往紅", "Youtube 演算法為什麼一直推薦我看「{{關鍵字}} 的交配過程」？"),
    ("高流量網紅往紅", "千萬別搜這個 hashtag，裡面全是 {{關鍵字}} 的黑歷史。"),
    ("高流量網紅往紅", "網美打卡聖地：只要拿著 {{關鍵字}} 拍照，讚數直接破萬。"),
    ("高流量網紅往紅", "知名實況主因為直播吃 {{關鍵字}} 遭到平台永久封鎖。"),
    ("高流量網紅往紅", "關於我轉生變成 {{關鍵字}} 這檔事。"),
    ("高流量網紅往紅", "這不是業配文，我是真心推薦大家把 {{關鍵字}} 塗在臉上。"),
    ("高流量網紅往紅", "炎上！某百萬 Youtuber 竟然公開歧視 {{關鍵字}}。"),

    # 時空旅人史萊姆
    ("時空旅人史萊姆", "秦始皇當年派徐福出海，其實是為了尋找長生不老的 {{關鍵字}}。"),
    ("時空旅人史萊姆", "如果當年牛頓是被 {{關鍵字}} 砸到頭，萬有引力就不會被發現了。"),
    ("時空旅人史萊姆", "穿越者警告：2030 年後，{{關鍵字}} 將會成為世界通用貨幣。"),
    ("時空旅人史萊姆", "考古學家在古墓裡發現了三千年前的 {{關鍵字}}，震驚學界。"),
    ("時空旅人史萊姆", "恐龍滅絕的真相：因為牠們消化不良吃不下 {{關鍵字}}。"),
    ("時空旅人史萊姆", "課本上沒寫的歷史：拿破崙滑鐵盧慘敗是因為忘了帶 {{關鍵字}}。"),
    ("時空旅人史萊姆", "哥倫布發現新大陸，第一句話大喊：「這裡竟然沒有 {{關鍵字}}！」"),
    ("時空旅人史萊姆", "文藝復興時期的畫家，其實都是看著 {{關鍵字}} 找靈感的。"),
    ("時空旅人史萊姆", "這把劍是亞瑟王從 {{關鍵字}} 裡面拔出來的。"),
    ("時空旅人史萊姆", "人類登陸月球的一小步，卻是 {{關鍵字}} 的一大步。"),

    # 零異現象好怕怕
    ("零異現象好怕怕", "筆仙筆仙，請告訴我，我的前世真的是 {{關鍵字}} 嗎？"),
    ("零異現象好怕怕", "半夜十二點對著鏡子削蘋果，你會看到未來的伴侶是 {{關鍵字}}。"),
    ("零異現象好怕怕", "這間凶宅很便宜，唯一的缺點是牆壁會流出 {{關鍵字}}。"),
    ("零異現象好怕怕", "貞子從電視機爬出來，手裡拿著 {{關鍵字}} 說要送給我。"),
    ("零異現象好怕怕", "路上看到紅包別亂撿，裡面可能裝著 {{關鍵字}} 的指甲。"),
    ("零異現象好怕怕", "殭屍來了別怕，只要停止呼吸並丟出 {{關鍵字}} 就能引開注意。"),
    ("零異現象好怕怕", "算命的說我印堂發黑，最近可能會被 {{關鍵字}} 纏上。"),
    ("零異現象好怕怕", "恐怖喔～恐怖到了極點！原來他背上背著一個 {{關鍵字}}。"),
    ("零異現象好怕怕", "地獄第十八層其實不可怕，可怕的是裡面只有 {{關鍵字}}。"),
    ("零異現象好怕怕", "我的守護靈說他不想幹了，因為我整天都在想 {{關鍵字}}。"),

    # 我沒錢我驕傲
    ("我沒錢我驕傲", "通貨膨脹太嚴重，現在連 {{關鍵字}} 我都得刷卡分期。"),
    ("我沒錢我驕傲", "月底吃土了，請問 {{關鍵字}} 加醬油好吃嗎？"),
    ("我沒錢我驕傲", "雙 11 購物節狂買了三箱 {{關鍵字}}，現在非常後悔。"),
    ("我沒錢我驕傲", "這家米其林餐廳的主廚推薦菜：法式舒肥 {{關鍵字}} 佐松露醬。"),
    ("我沒錢我驕傲", "我把靈魂賣給了路西法，換來了一張 {{關鍵字}} 的折價券。"),
    ("我沒錢我驕傲", "醫生，我最近胃口不好，看什麼都像 {{關鍵字}}。"),
    ("我沒錢我驕傲", "全聯先生說：實在真便宜，連 {{關鍵字}} 都有賣。"),
    ("我沒錢我驕傲", "為了買房，我決定從今天開始戒掉 {{關鍵字}}。"),
    ("我沒錢我驕傲", "外送員偷吃了我的餐點，還留下一張紙條寫著「{{關鍵字}} 真香」。"),
    ("我沒錢我驕傲", "只要有心，人人都可以用 {{關鍵字}} 煮出一桌好菜。")
]
